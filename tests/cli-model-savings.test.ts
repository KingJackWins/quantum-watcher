...
function readConfig(home: string): Promise<Record<string, unknown>> {
  return readFile(join(home, '.config', 'quantum-watcher', 'config.json'), 'utf-8')
    .then(raw => JSON.parse(raw) as Record<string, unknown>)
}

describe('quantum-watcher model-savings command', () => {
  it('saves, lists, and removes a local-model savings mapping', async () => {
    const home = await mkdtemp(join(tmpdir(), 'quantum-watcher-cli-savings-'))
    try {
