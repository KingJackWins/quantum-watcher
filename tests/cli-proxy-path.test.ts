...
async function makeHome(): Promise<string> {
  const home = await mkdtemp(join(tmpdir(), 'quantum-watcher-proxy-cli-'))
  homes.push(home)
  return home
}
...
const configPath = (home: string) => join(home, '.config', 'quantum-watcher', 'config.json')
...
async function writeConfig(home: string, obj: unknown): Promise<void> {
  await mkdir(join(home, '.config', 'quantum-watcher'), { recursive: true })
  await writeFile(configPath(home), JSON.stringify(obj), 'utf-8')
}

describe('quantum-watcher proxy-path CLI', () => {
...
describe('quantum-watcher report --format json: proxy overview', () => {
