...
describe('quantum-watcher plan command', () => {
  it('persists provider-keyed plans and clears on reset', async () => {
    const home = await mkdtemp(join(tmpdir(), 'quantum-watcher-cli-plan-'))
    ...
    try {
      ...
      const configPath = join(home, '.config', 'quantum-watcher', 'config.json')
