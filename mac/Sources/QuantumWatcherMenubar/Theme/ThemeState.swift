...
final class ThemeState {
    ...
    var preset: AccentPreset {
        didSet { UserDefaults.standard.set(preset.rawValue, forKey: "QuantumWatcherAccentPreset") }
    }

    private init() {
        let saved = UserDefaults.standard.string(forKey: "QuantumWatcherAccentPreset") ?? ""
        self.preset = AccentPreset(rawValue: saved) ?? .green
    }
}
