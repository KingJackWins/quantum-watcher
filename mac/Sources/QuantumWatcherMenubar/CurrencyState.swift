...
actor FXRateCache {
    ...
    private var cacheFilePath: String {
        ...
        return base
            .appendingPathComponent("quantum-watcher-mac", isDirectory: true)
    ...
    func rate(for code: String) async -> Double? {
        ...
        do {
            ...
            guard let fresh = decoded.rates[code],
                ...
                NSLog("QuantumWatcher: discarding out-of-band FX rate for \(code)")
                return entries[code]?.rate
            }
...
/// Reads and writes the CLI's persisted currency config (~/.config/quantum-watcher/config.json).
/// Uses an on-disk flock so a concurrent `quantum-watcher currency ...` invocation from a terminal
...
enum CLICurrencyConfig {
    private static var configDir: String {
        (NSHomeDirectory() as NSString).appendingPathComponent(".config/quantum-watcher")
    }
    ...
    static func persist(code: String) {
        do {
            ...
            NSLog("QuantumWatcher: failed to persist currency config: \(error)")
        }
    }
}

struct QuantumWatcherCLIConfigStore {
    ...
    private var configDir: String {
        (homeDirectory as NSString).appendingPathComponent(".config/quantum-watcher")
    }
    ...
    func persistDevinAcuUsdRate(_ rate: Double) {
        ...
        do {
            ...
            NSLog("QuantumWatcher: failed to persist Devin ACU config: \(error)")
        }
    }
}

enum CLIDevinConfig {
    private static let store = QuantumWatcherCLIConfigStore()
