...
private let starBannerGitHubURL = URL(string: "https://github.com/KingJackWins/quantum-watcher")!
...
struct StarBanner: View {
    @AppStorage("quantumwatcher.starBannerDismissed") private var dismissed: Bool = false
...
struct FooterBar: View {
    ...
    /// Runs `quantum-watcher export` directly into ~/Downloads and reveals the result in Finder. CSV
    ...
    private func runExport(format: ExportFormat) {
        Task {
            ...
            let base = "quantum-watcher-\(formatter.string(from: Date()))"
            let outputPath = (downloads as NSString).appendingPathComponent(base + format.suffix)

            let process = QuantumWatcherCLI.makeProcess(subcommand: [
                ...
                process.terminationHandler = { proc in
                    Task { @MainActor in
                        if proc.terminationStatus == 0 {
                            ...
                            NSLog("QuantumWatcher: \(fmt.cliName.uppercased()) export exited with status \(proc.terminationStatus)")
                        }
                    }
                }
                try process.run()
            } catch {
                NSLog("QuantumWatcher: \(format.cliName.uppercased()) export failed: \(error)")
            }
        }
    }
    ...
    /// Instant-feeling currency switch. Updates the symbol and any cached FX rate on the main
     ...
     /// background. CLI config is persisted so other quantum-watcher commands stay in sync.
