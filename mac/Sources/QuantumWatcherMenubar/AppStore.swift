...
private let menubarPeriodDefaultsKey = "QuantumWatcherMenubarPeriod"
...
final class AppStore {
    ...
    var displayMetric: DisplayMetric = DisplayMetric(rawValue: UserDefaults.standard.string(forKey: "QuantumWatcherDisplayMetric") ?? "") ?? .cost {
        didSet { UserDefaults.standard.set(displayMetric.rawValue, forKey: "QuantumWatcherDisplayMetric") }
    }
    ...
    func clearStaleLoadingIfNeeded() -> Bool {
        ...
        for (key, started) in staleLoading {
            NSLog("QuantumWatcher: loading stuck for %ds on %@/%@ — auto-clearing",
                  Int(now.timeIntervalSince(started)), key.label, key.provider.rawValue)
            loadingCountsByKey[key] = nil
            loadingStartedAtByKey[key] = nil
            inFlightKeys[key] = nil
            if cache[key] == nil {
                lastErrorByKey[key] = "Refresh took longer than expected. Quantum Watcher will keep retrying in the background."
            }
        }
        for (key, insertedAt) in staleInFlight {
            NSLog("QuantumWatcher: orphaned in-flight key stuck for %ds on %@/%@ — clearing",
    ...
    private func invalidateStaleDayCache() {
        ...
        if cacheDate != today {
            ...
            lastErrorByKey.removeAll()
            cacheDate = today
            NSLog("QuantumWatcher: reset menubar payload cache for new day %@", today)
        }
    }
    ...
    func setRecoveryExhausted(for label: String) {
        lastErrorByKey[currentKey] = "Could not load \(label). Check that the quantum-watcher CLI is installed and working."
    }
    ...
    private func refresh(key: PayloadCacheKey, includeOptimize: Bool, force: Bool = false, showLoading: Bool = false) async {
        ...
        if staleSeconds.isFinite, staleSeconds > 120 {
            NSLog("QuantumWatcher: refresh attempt for stale key \(key.label)/\(key.provider.rawValue) — last success was \(Int(staleSeconds))s ago")
        }
        defer {
        ...
        do {
            ...
            if generationAtStart != payloadRefreshGeneration {
                NSLog("QuantumWatcher: dropping fetch result for \(key.label)/\(key.provider.rawValue) — refresh pipeline reset mid-fetch")
                return
            }
            if Task.isCancelled {
                ...
                NSLog("QuantumWatcher: fetch for \(key.label)/\(key.provider.rawValue) cancelled before result was applied")
                return
            }
            ...
            if cacheDate != cacheDateAtStart || cacheDate != currentCacheDate() {
                invalidateStaleDayCache()
                NSLog("QuantumWatcher: dropping fetch result for \(key.label)/\(key.provider.rawValue) — calendar rolled mid-fetch")
                return
            }
            ...
            if Task.isCancelled { return }
            NSLog("QuantumWatcher: fetch failed for \(key.label)/\(key.provider.rawValue): \(error)")
            if includeOptimize, cache[key] == nil {
                do {
                    ...
                    if Task.isCancelled { return }
                    NSLog("QuantumWatcher: fallback fetch also failed: \(error)")
                }
            }
    ...
    func refreshQuietly(period: Period, day: String? = nil, force: Bool = false) async {
        ...
        if day == nil && period == .today, let age = todayPayloadAgeSeconds, age > 120 {
            NSLog("QuantumWatcher: refreshing stale today status payload after %ds", age)
        }
        defer {
            inFlightKeys[key] = nil
        }
        do {
            ...
            if generationAtStart != payloadRefreshGeneration {
                NSLog("QuantumWatcher: dropping quiet fetch result for \(key.label) — refresh pipeline reset mid-fetch")
                return
            }
            ...
            NSLog("QuantumWatcher: quiet refresh failed for \(key.label): \(error)")
        }
    }
    ...
    func disconnectSubscription() {
        ...
        NotificationCenter.default.post(name: .quantumWatcherSubscriptionDisconnected, object: nil)
    }
...
extension Notification.Name {
    static let quantumWatcherSubscriptionDisconnected = Notification.Name("com.quantumwatcher.subscriptionDisconnected")
}
