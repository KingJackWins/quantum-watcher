...
/// Connect we cache a copy under ~/Library/Application Support/QuantumWatcher so
/// we keep using rotated tokens after refresh.
enum CodexCredentialStore {
    private static let bootstrapCompletedKey = "quantumwatcher.codex.bootstrapCompleted"
    ...
    private static func refreshAndPersist(record: CredentialRecord) async throws -> CredentialRecord {
        ...
        do {
            ...
            NSLog("QuantumWatcher: codex cache write failed during refresh rotation: %@", String(describing: error))
        }
        return updated
    }
}
