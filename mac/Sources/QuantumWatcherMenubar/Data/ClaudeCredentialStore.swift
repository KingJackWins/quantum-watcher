...
///      entry once, then maintain `com.quantumwatcher.menubar.claude.oauth.v1` in
...
enum ClaudeCredentialStore {
    private static let bootstrapCompletedKey = "quantumwatcher.claude.bootstrapCompleted"
    ...
    private static func refreshAndPersist(record: CredentialRecord) async throws -> CredentialRecord {
        ...
        do {
            ...
            NSLog("QuantumWatcher: cache write failed during refresh rotation: %@", String(describing: error))
        }
        return updated
    }
}
