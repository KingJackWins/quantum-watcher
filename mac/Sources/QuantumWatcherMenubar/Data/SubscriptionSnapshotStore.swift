...
private func snapshotsCacheDir() -> String {
    return ProcessInfo.processInfo.environment["QUANTUM_WATCHER_CACHE_DIR"]
        ?? (NSHomeDirectory() as NSString).appendingPathComponent(".cache/quantum-watcher")
}
    ...
    static func record(_ snapshot: SubscriptionSnapshot) async {
        await SnapshotLock.shared.run {
            do {
                ...
                NSLog("QuantumWatcher: snapshot record failed: \(error)")
            }
        }
    }
