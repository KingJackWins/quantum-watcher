...
private let releasesAPI = "https://api.github.com/repos/KingJackWins/quantum-watcher/releases?per_page=20"
...
final class UpdateChecker {
    ...
    var cliUpdateCommand: String {
        let argv = QuantumWatcherCLI.baseArgv()
        let path = argv.first ?? ""
        if path.contains("/homebrew/") { return "brew upgrade quantum-watcher" }
        return "npm update -g quantum-watcher"
    }
    ...
    func check() async {
        ...
        request.setValue("quantum-watcher-menubar-updater", forHTTPHeaderField: "User-Agent")
        ...
        do {
            ...
            let version = resolved.asset.name
                .replacingOccurrences(of: "QuantumWatcherMenubar-", with: "")
            ...
            NSLog("QuantumWatcher: update check failed: \(error)")
        }
    }
    ...
    nonisolated static func queryInstalledCliVersion() -> String? {
        let process = QuantumWatcherCLI.makeProcess(subcommand: ["--version"])
        let pipe = Pipe()
    ...
    nonisolated static func resolveLatestMenubarRelease(in releases: [GitHubRelease]) -> (release: GitHubRelease, asset: GitHubAsset)? {
        for release in releases where release.tag_name.hasPrefix("mac-v") {
            guard let asset = release.assets.first(where: {
                $0.name.hasPrefix("QuantumWatcherMenubar-v") && $0.name.hasSuffix(".zip")
            }) else { continue }
    ...
    func performUpdate() {
        ...
        let process = QuantumWatcherCLI.makeProcess(subcommand: ["menubar", "--force"])
        let errPipe = Pipe()
        ...
        let timeoutTask = Task.detached(priority: .utility) {
            ...
            if process.isRunning {
                NSLog("QuantumWatcher: update subprocess timed out after %llus - terminating", updateTimeoutSeconds)
                process.terminate()
            }
        }
        ...
        process.terminationHandler = { [weak self] proc in
            ...
            Task { @MainActor in
                ...
                if proc.terminationStatus != 0 {
                    ...
                    NSLog("QuantumWatcher: update failed (exit \(proc.terminationStatus)): \(stderr)")
                } else {
        ...
        do {
            ...
            NSLog("QuantumWatcher: update spawn failed: \(error)")
        }
    }
