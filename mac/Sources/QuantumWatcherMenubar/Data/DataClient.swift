...
/// Runs the CLI via argv (no shell interpretation). See `QuantumWatcherCLI` for why we never route
...
struct DataClient {
    ...
    private static func drain(_ handle: FileHandle, limit: Int) async -> Data {
        ...
        if flags >= 0 {
            ...
            NSLog("QuantumWatcher: fcntl F_GETFL failed on fd %d, drain may block", fd)
        }
        ...
        while buffer.count < limit && !Task.isCancelled {
            ...
            if n > 0 {
                ...
                NSLog("QuantumWatcher: drain read() failed on fd %d: errno %d", fd, errno)
                break
            }
        }
        return buffer
