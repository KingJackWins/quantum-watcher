...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private func observeSubscriptionDisconnect() {
        NotificationCenter.default.addObserver(
            forName: .quantumWatcherSubscriptionDisconnected,
            object: nil,
            queue: .main
        ) { [weak self] _ in
    ...
    private func recoverRefreshPipelineAfterInterruption(resetLoading: Bool, clearCache: Bool = false, reason: String) {
        ...
        if refreshTimer == nil || loopAge > refreshLoopWatchdogSeconds {
            if refreshTimer != nil {
                NSLog("QuantumWatcher: refresh loop stale for %ds after %@ - restarting", Int(loopAge), reason)
            }
            startRefreshLoop(forceQuotaOnStart: false)
        } else {
...
"""
    ...
    private func registerLoginItemIfNeeded() {
        ...
        do {
            ...
            NSLog("QuantumWatcher: Login item registration failed: \(error)")
        }
    }
    ...
    private func clearStaleForceRefreshIfNeeded(now: Date = Date()) -> Bool {
        if forceRefreshTask != nil {
            guard let started = forceRefreshStartedAt else {
                NSLog("QuantumWatcher: force refresh task had no start timestamp - clearing")
                forceRefreshTask?.cancel()
                forceRefreshTask = nil
                forceRefreshGeneration &+= 1
                store.resetLoadingState()
            ...
            NSLog("QuantumWatcher: force refresh stuck for %ds - cancelling and restarting", Int(elapsed))
            forceRefreshTask?.cancel()
            forceRefreshTask = nil
            forceRefreshStartedAt = nil
            forceRefreshGeneration &+= 1
    ...
    private func clearStaleStatusPayloadRefreshIfNeeded(now: Date = Date()) -> Bool {
        if statusPayloadRefreshTask != nil {
            guard let started = statusPayloadRefreshStartedAt else {
                NSLog("QuantumWatcher: today status refresh task had no start timestamp - clearing")
                statusPayloadRefreshTask?.cancel()
                statusPayloadRefreshTask = nil
                statusPayloadRefreshGeneration &+= 1
                return true
            ...
            NSLog("QuantumWatcher: today status refresh stuck for %ds - cancelling", Int(elapsed))
            statusPayloadRefreshTask?.cancel()
            statusPayloadRefreshTask = nil
            statusPayloadRefreshStartedAt = nil
            statusPayloadRefreshGeneration &+= 1
    ...
    private func refreshStatusPayloadIfNeeded(reason: String, force: Bool = false) {
        ...
        if let age = store.menubarPayloadAgeSeconds, age > 120 {
            NSLog("QuantumWatcher: status payload stale for %ds on %@ refresh", age, reason)
        }
    ...
    /// Loads the currency code persisted by `quantum-watcher currency` so a relaunch picks up where
    ...
    private var isCompact: Bool {
        UserDefaults.standard.bool(forKey: "QuantumWatcherMenubarCompact")
    }
    ...
    private func setupStatusItem() {
        ...
        let flame = NSImage(systemSymbolName: "flame.fill", accessibilityDescription: "Quantum Watcher")?
    ...
    private func showContextMenu(from button: NSStatusBarButton) {
        ...
        menu.addItem(.separator())
        let quitItem = NSMenuItem(title: "Quit Quantum Watcher", action: #selector(quitApp), keyEquivalent: "q")
    ...
    @objc private func openSettings() {
        ...
        window.title = "Quantum Watcher Settings"
    ...
    private func quantumWatcherAlertIcon() -> NSImage? {
        let config = NSImage.SymbolConfiguration(pointSize: 32, weight: .medium)
        guard let symbol = NSImage(systemSymbolName: "flame.fill", accessibilityDescription: "Quantum Watcher")?
    ...
    @objc private func checkForUpdates() {
        Task {
            ...
            let alert = NSAlert()
            alert.icon = quantumWatcherAlertIcon()
