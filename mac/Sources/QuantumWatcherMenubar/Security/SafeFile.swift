...
///
/// Every cache file we touch (`~/Library/Caches/quantum-watcher-mac/fx-rates.json`,
/// `~/.cache/quantum-watcher/subscription-snapshots.json`, `~/.config/quantum-watcher/config.json`) is a
...
enum SafeFile {
    ...
    static func write(_ data: Data, to path: String, mode: mode_t = 0o600) throws {
        ...
        let tmpPath = parent + "/.quantum-watcher-" + UUID().uuidString + ".tmp"
    ...
    /// quantum-watcher processes (the CLI running in a terminal, say) block on the same file instead
