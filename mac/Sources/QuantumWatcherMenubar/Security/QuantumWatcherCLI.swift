...
/// Single entry point for spawning the `quantum-watcher` CLI. All callers route through here so the
...
enum QuantumWatcherCLI {
    ...
    /// newlines) so a malicious `QUANTUM_WATCHER_BIN="quantum-watcher; rm -rf ~"` can't slip through.
    ...
    private static let userNodePaths: [String] = {
        ...
        if let entries = try? FileManager.default.contentsOfDirectory(atPath: versionsDir) {
            for entry in entries.sorted().reversed() {
                ...
                if FileManager.default.isExecutableFile(atPath: "\(bin)/quantum-watcher") {
                    paths.append(bin)
                    break
                }
            }
    ...
    private static let persistedPathFilename = "quantum-watcher-cli-path.v1"

    /// Returns the argv that launches the CLI. Dev override via `QUANTUM_WATCHER_BIN` is honoured only
    /// if every whitespace-delimited token passes `safeArgPattern`. Otherwise falls back to the
    /// plain `quantum-watcher` name (resolved via PATH).
    static func baseArgv() -> [String] {
        if ProcessInfo.processInfo.environment["QUANTUM_WATCHER_ALLOW_DEV_BIN"] == "1",
           let raw = ProcessInfo.processInfo.environment["QUANTUM_WATCHER_BIN"],
           !raw.isEmpty
        {
            let parts = raw.split(separator: " ", omittingEmptySubsequences: true).map(String.init)
            guard parts.allSatisfy(isSafe) else {
                NSLog("QuantumWatcher: refusing unsafe QUANTUM_WATCHER_BIN; using installed quantum-watcher")
    ...
    private static func installedArgv() -> [String] {
        ...
        for candidate in (additionalPathEntries + userNodePaths).map({ "\($0)/quantum-watcher" }) {
            if isSafe(candidate), FileManager.default.isExecutableFile(atPath: candidate) {
                return [candidate]
            }
        }
        return ["quantum-watcher"]
    }
    ...
    private static func persistedCLIPath() -> String? {
        ...
        let url = support
            .appendingPathComponent("QuantumWatcher", isDirectory: true)
    ...
    static func makeProcess(subcommand: [String]) -> Process {
        ...
        // quantum-watcher subprocess parses 5-10x slower than the same command run from a
