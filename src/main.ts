...
program
  ...
  .description('Track a local model as "savings" rather than cost. Maps a local-model name to a paid baseline so the dashboard can show what the same tokens would have cost on the baseline (e.g. quantum-watcher model-savings "llama3.1:8b" gpt-4o). The local call itself still costs $0 — actual cost is left untouched.')
  ...
  .action(async (local?: string, baseline?: string, opts?: { remove?: string; list?: boolean }) => {
    ...
    if (opts?.list || (!local && !opts?.remove)) {
      ...
      if (entries.length === 0) {
        ...
        console.log(`  Config: ${getConfigFilePath()}`)
        console.log('  Add one with: quantum-watcher model-savings <local-model> <baseline-model>\n')
      } else {
    ...
    if (!local || !baseline) {
      console.error('\n  Usage: quantum-watcher model-savings <local-model> <baseline-model>\n')
      process.exitCode = 1
      return
    }
  ...
  .action(async (path?: string, opts?: { remove?: string; list?: boolean }) => {
    ...
    if (opts?.list || (!path && !opts?.remove)) {
      if (paths.length === 0) {
        ...
        console.log(`  Config: ${getConfigFilePath()}`)
        console.log('  Add one with: quantum-watcher proxy-path <project-dir>\n')
      } else {
    ...
    if (!path) {
      console.error('\n  Usage: quantum-watcher proxy-path <project-dir>\n')
      process.exitCode = 1
      return
    }
    ...
    if (!isAbsolute(trimmed) || normalizeProxyPath(trimmed) === '') {
      ...
      console.error('  quantum-watcher matches sessions by their recorded absolute cwd; the')
