...
const INSTRUCTIONS =
  'QuantumWatcher exposes local AI-coding spend data. Use get_usage for spend/usage and breakdowns (fast); ' +
...
export function createServer(deps: { version: string; aggregate?: Aggregate }): McpServer {
  ...
  const server = new McpServer({ name: 'quantum-watcher', version: deps.version }, { instructions: INSTRUCTIONS })

  server.registerTool(
    'get_usage',
    {
      title: 'QuantumWatcher — usage & cost',
      description:
      ...
      annotations: { title: 'QuantumWatcher — usage & cost', readOnlyHint: true, openWorldHint: false, idempotentHint: true },
    },
    async ({ period, by, limit, include_project_names }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `quantum-watcher: failed to read usage — ${err instanceof Error ? err.message : String(err)}` }],
  ...
  server.registerTool(
    'get_savings',
    {
      title: 'QuantumWatcher — savings opportunities',
      description:
      ...
      annotations: { title: 'QuantumWatcher — savings opportunities', readOnlyHint: true, openWorldHint: false, idempotentHint: true },
    },
    async ({ period, include_project_names }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `quantum-watcher: failed to compute savings — ${err instanceof Error ? err.message : String(err)}` }],
