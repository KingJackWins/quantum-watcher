...
async function isCodeburnExportFolder(path: string): Promise<boolean> {
  // Check new marker first, then legacy .quantum-watcher-export for backward compat
  const markerStat = await stat(join(path, EXPORT_MARKER_FILE)).catch(() => null)
  if (markerStat?.isFile()) return true
  const legacyStat = await stat(join(path, '.quantum-watcher-export')).catch(() => null)
...
export async function exportJson(periods: PeriodExport[], outputPath: string): Promise<string> {
  ...
  if (existing?.isFile()) {
    ...
    try {
      ...
      if (!head.includes('"schema": "quantum-watcher.export.v') && !head.includes('"schema": "quantum-watcher.export.v')) {
        throw new Error(
