...
export function calculateCost(
  ...
  if (!costs) {
    if (shouldWarnAboutUnknownModel(model)) {
      ...
      const aliasHint = `Map it with: quantum-watcher model-alias "${safeName}" <known-model>, or track local-model savings with: quantum-watcher model-savings "${safeName}" <baseline-model>`
