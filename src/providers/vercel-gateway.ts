  ...
  try {
    ...
    if (!res.ok) {
      ...
      process.stderr.write(
        `quantum-watcher: Vercel AI Gateway report failed (HTTP ${res.status}). ` +
    ...
    process.stderr.write(
      `quantum-watcher: Vercel AI Gateway report unreachable (${err instanceof Error ? err.message : String(err)}).\n`,
    )
    return []
  }
}
