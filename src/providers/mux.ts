...
function getMuxRoot(override?: string): string {
  ...
  const quantumWatcherOverride = process.env['CODEBURN_MUX_DIR']
  if (quantumWatcherOverride) return resolve(expandHome(quantumWatcherOverride))
  ...
  return {
    async *parse(): AsyncGenerator<ParsedProviderCall> {
      ...
      for await (const line of readSessionLines(source.path)) {
        ...
        // outputTokens inclusive of reasoning; decompose to quantum-watcher's
        ...
        // Strip the "provider:" prefix — quantum-watcher's getCanonicalName only strips
