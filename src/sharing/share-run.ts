...
export async function runShareServer(opts: { port: number; pair: boolean; always: boolean }): Promise<void> {
  ...
  process.stdout.write(`  On your other Mac, run:  quantum-watcher devices add\n`)
  if (opts.pair) {
    const pin = server.openPairing(120_000)
    process.stdout.write(`\n  Manual fallback (if discovery is blocked):\n`)
    process.stdout.write(`    quantum-watcher devices add ${ip}:${port} --pin ${pin}\n`)
  }
  ...
  if (!opts.always) {
    ...
    const timer = setInterval(() => {
      if (Date.now() - last > IDLE_TIMEOUT_MS) {
        process.stdout.write('\n  Idle, stopping share. Run `quantum-watcher share` again when you need it.\n')
        process.exit(0)
      }
    }, 30_000)
    timer.unref()
